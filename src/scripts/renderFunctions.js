...
import { textSize } from "./helpers.js";
...
const renderList = async (input) => {
  ...
  if (recipes.firstElementChild) recipes.firstElementChild.remove();
  ...
  // after list is created get recipes from api if input from user is valid
...
const renderRecipeAbout = async (recipeID, bookmarksList) => {
  ...
  const ingredientsList = recipeData.ingredients.map(
    (ingredient) =>
      `<li class="ingredient"><strong>${ingredient.quantity ?? ""} ${
        ingredient.unit ?? ""
      }</strong> ${ingredient.description}</li>`
  );
